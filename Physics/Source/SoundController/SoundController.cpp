...
bool CSoundController::Init(void)
{
	...
	cSoundEngine->setSoundVolume(0.05);
	return true;
}
									...
									vec3df vec3dfSoundPos)
{
	...
	else if (eSoundType == CSoundInfo::SOUNDTYPE::BGM)
		cSoundInfo->Init(ID, pSoundSource, true, eSoundType);
	else
...
void CSoundController::PlayBGM(const int ID) {
	CSoundInfo* pSoundInfo = GetSound(ID);
	if (!pSoundInfo) {
		cout << "Sound #" << ID << " for bgm is not playable." << endl;
		return;
	}
	if (pSoundInfo->GetSoundType() == CSoundInfo::SOUNDTYPE::BGM)
	{
		cSoundEngine->play2D(pSoundInfo->GetSound(),
			true);
	}
...
bool CSoundController::setVolumeID(const int ID, float volume)
{
	ISoundSource* pISoundSource = GetSound(ID)->GetSound();
	if (pISoundSource == nullptr)
	{
